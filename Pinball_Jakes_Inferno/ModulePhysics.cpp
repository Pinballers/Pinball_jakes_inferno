...
update_status ModulePhysics::PostUpdate()
{
	...
	for (b2Body* b = world->GetBodyList(); b; b = b->GetNext())
	{
		for (b2Fixture* f = b->GetFixtureList(); f; f = f->GetNext())
		{
			...
			case b2Shape::e_circle:
			{
				...
				App->renderer->DrawCircle(METERS_TO_PIXELS(pos.x * SCREEN_SIZE), METERS_TO_PIXELS(pos.y * SCREEN_SIZE), METERS_TO_PIXELS(shape->m_radius * SCREEN_SIZE), 255, 0, 0);
			}
			break;
	...
	if (mouse_joint && clicked && App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_REPEAT)
	{
	...
	if (mouse_joint && clicked && App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_UP)
	{
